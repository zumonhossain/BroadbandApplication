            ...
            <div class="card card-outline-info">
                ...
                <div class="card-body">
                    <form action="{{ route('product_purchase_update_form') }}" method="POST" class="form-horizontal">
                        ...
                        <input type="hidden" name="product_purchase_id" value="{{ $productPurchase->product_purchase_id }}">
                        ...
                        <div class="row row-sm">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="purchase_form_id">
                                        <option value="">-- Select Purchase Form --</option>
                                            <option value="1" {{ $productPurchase->purchase_form_id == 1 ? 'selected' : '' }}>Link-3 Tech</option>
                                            <option value="2" {{ $productPurchase->purchase_form_id == 2 ? 'selected' : '' }}>Layer -3</option>
                                            <option value="3" {{ $productPurchase->purchase_form_id == 3 ? 'selected' : '' }}>Fiber @ home</option>
                                            <option value="4" {{ $productPurchase->purchase_form_id == 4 ? 'selected' : '' }}>Summit-1</option>
                                    </select>
                                    @error('purchase_form_id')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="month_id">
                                        ...
                                        @foreach($months as $month)
                                            <option value="{{ $month->month_id }}" {{ $month->month_id == $productPurchase->month_id ? 'selected': '' }}>{{ $month->month_name_en }}</option>
                                        @endforeach
                                    </select>
                                    @error('month_id')
                                        <span class="text-danger">{{ $message }}</span>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="year">
                                        @foreach($years as $year)
                                            <option value="{{ $year->product_purchase_id }}" >{{ $year->year }}</option>
                                        @endforeach
