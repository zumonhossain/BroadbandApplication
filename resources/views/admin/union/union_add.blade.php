                ...
                <div class="card-body">
                    <form action="{{ route('union_insert_form') }}" method="POST" class="form-horizontal">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-7">
                                <input type="text" name="union_name" class="form-control" placeholder="Union Name" value="{{ old('union_name') }}">
                                @error('union_name')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                ...
                <div class="card-body">
                    <table class="table table-bordered table-striped table-hover custom_table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $union->division->division_name }}</td>
                                <td>{{ $union->district->district_name }}</td>
                                <td>{{ $union->upazila->upazila_name }}</td>
                                <td>{{ $union->union_name }}</td>
                                <td>
                                    <a class="btn-info edit-icon" href="{{ route('union_edit_form',$union->union_id) }}"><i class="mdi mdi-table-edit"></i></a>
                                    <a class="btn-danger delete-icon" id="softDelete" data-toggle="modal" data-target="#softDelModal" data-id="{{ $union->union_id }}" href="#"><i class="mdi mdi-delete"></i></a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
